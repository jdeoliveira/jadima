...
public class Delete extends Task {
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            Target t = new Target();
            t.setProject(p);
            t.setName("admin-delete");
            t.addTask(task);
            p.addTarget("admin-delete", t);
    ...
    public void execute() {
        ...
        try {
            //Vemos si lo que quiere es borrar una version o una libreria
            if ((majorVersion == -1) && (minorVersion == -1)) {
                //quiere borrar la libreria, porque no indico numero de version
                
                log("Deleting library " + libraryName + " on repository " + repositoryName, Project.MSG_INFO);
                RepositoryBroker.deleteLibrary(
                    repositoryName,
                    libraryName);
                
                log("Library " + libraryName + " successfuly deleted on repository " + repositoryName, Project.MSG_INFO);
                
            } else {
                //indico numero de version, quiere BORRAR la version
                
                log("Deleting version " + libraryName + "-" + majorVersion + "." + minorVersion + " on repository " + repositoryName, Project.MSG_INFO);
                
                RepositoryBroker.deleteVersion(
                    repositoryName,
                    libraryName,
                    majorVersion,
                    minorVersion);
                ...
                log("Version " + libraryName + "-" + majorVersion + "." + minorVersion + " successfuly deleted on repository " + repositoryName, Project.MSG_INFO);
            }
    ...
    public static void showHelpAndExit() {
        ...
        HelpFormatter formatter = new HelpFormatter();
            formatter.printHelp(
                "jdm", 
                "\n" + 
                "JaDiMa Command Line Tools :: Library/Version Deleter v1.0\n" +
                "=========================================================\n", 
                options, 
                "\n" + 
